...
from butterfly_dataset import AnimalDataset



BASE_DIR = "Animals"
DATA_ROOT = os.path.join(BASE_DIR, "butterfly-class")  # ✅ no backslash escape
...
# 1) Read image paths and labels from csv

def load_image_paths_and_labels(csv_path, image_dir):
...
def create_dataloaders():
    # Load labeled data
    all_paths, all_labels = load_image_paths_and_labels(TRAIN_CSV, TRAIN_DIR)
    ...
    all_labels_idx = [label_to_idx[lbl] for lbl in all_labels]
    ...
    def select(idxs):
        return [all_paths[i] for i in idxs], [all_labels_idx[i] for i in idxs]
    ...
    # Create datasets
    train_dataset = AnimalDataset(train_paths, train_labels, None, transform=train_transform)
    val_dataset = AnimalDataset(val_paths, val_labels, None, transform=val_transform)
...
def train():
    ...
    torch.save(
        ...
        "butterfly_model_local.pth",
    )
    print("Saved model to butterfly_model_local.pth")
